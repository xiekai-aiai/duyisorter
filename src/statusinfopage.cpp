...
void StatusInfoPage::getAlarmIndex(int index)
{
    ...
    if (struCnfg.nAlarmEnable[index])
    {
        ...
        if (ret == QDialog::Accepted)
        {
            ...
            if (str == "002690")
            {
                ...
                LOG_INFO_STM("alarm:" << getAlarmName(index).toStdString() << ", status:" << myLan.off.toStdString());
            }
            else
            {
                alarmEnableBtn[index]->setChecked(struCnfg.nAlarmEnable[index]);
    ...
    else
    {
        ...
        LOG_INFO_STM("alarm:" << getAlarmName(index).toStdString() << ", status:" << myLan.on.toStdString());
    }
