 ...
 *              All rights reserved.
 * \file        setmaterialsens.cpp
 * \brief       彩色机型设置灵敏度界面源文件
 * \date        2015.01.14
 */
...
//    statusListItem[1] = new myListWidgetItem(myLan.material_shape, myIcon.Sorter_Shape, QSize(BTN_WIDTH,80));
    ...
    // 智能保留参数页面
    m_pageAIRsv = new PageReserved();
    m_pageAIRsv->setComponentsHidden(QList<int>() << PageReserved::COMPONENT_TABBAR);

    // 保留算法页面
    m_pageGeneralRsv = new QWidget();
    createRsvPage();
...
//    m_sensBiasBtn  = new myPushButton("",myIcon.Action_Configure, true, true, this);
    ...
    // 页面布局
    sensPageGridLayout = new QGridLayout(this);
    sensPageHBLayout1 = new QHBoxLayout();
    sensPageHBLayout1->addWidget(tabBar, 0, Qt::AlignHCenter);
    sensPageGridLayout->addLayout(sensPageHBLayout1, 0, 0, 1, 1);

    sensPageHBLayout2 = new QHBoxLayout();
    sensPageHBLayout2->addWidget(listWidget);
    ...
    if (LCD_WIDTH == 640) {
        ...
        sensPageGridLayout->setMargin(0);
    }
...
/**
 * @brief setMaterialSens::onCancelBtnClickedSlt
 */
...
/**
 * @brief setMaterialSens::updateAll
 * 更新整个页面
 */
void setMaterialSens::updateAll()
{
    updateListWidget();
    updateGeneralList();
    updateAIList();
...
/**
 * @brief setMaterialSens::upTabBar
 * 更新tabBar
 */
...
/**
 * @brief setMaterialSens::updateListWidget
 * 更新总体算法列表，传统，智能，形状
 */
void setMaterialSens::updateListWidget()
{
    int i;
    bool bFlag = false;
...
/* 响应灵敏度按钮按下事件 */
void setMaterialSens::onSensBiasBtnPressed()
{
    emit goToSensBiasPageSig();
}

/* 刷新灵敏度偏置按钮状态 */
void setMaterialSens::updateSensBiasBtn()
{
    /* 灵敏度偏置按钮的显示开关 */
    bool bNoArith = true;
    /* 若算法列表为空则不显示该按钮 */
...
/**
 * @brief setMaterialSens::createGeneralPage
 * 创建传统算法页面
 */
void setMaterialSens::createGeneralPage()
{
    QString strGeneralSens;
    QString strGeneralRow;
    QString strGeneralPercent;
    ...
    for (int i = 0; i < MAX_GENERAL;i++) {
        ...
        generalHBoxLayout[i]->addWidget(generalRowLabel[i]);
        generalHBoxLayout[i]->addWidget(generalRowLbe[i]);
        generalHBoxLayout[i]->addSpacing(50);
...
/**
 * @brief setMaterialSens::updateGeneralList
 * 更新传统算法列表
 */
void setMaterialSens::updateGeneralList()
{
    QString strName;
    QString strGeneralList;
    QString strGeneralSens;
    QString strGeneralRow;
    QString strGeneralPercent;
    int color;
...
/**
 * @brief setMaterialSens::getGeneralIndex
 * @param index
 * 修改各参数值
 */
void setMaterialSens::getGeneralIndex(int index)
{
    int ret;
    inputType type;
    int nMin, nMax;
    double nSens[MAX_GENERAL];
    QString str;
...
/**
 * @brief setMaterialSens::createShapePage
 * 创建形状算法列表
 */
void setMaterialSens::createShapePage()
{
...
//            shapeSensListCbx[i] = new myGroupBox(myString.sArithmeticName[ARITH_PISTACHIO], pageShape);
    ...
    /*更新形状算法列表*/
    updateShapeList();
    connect(shapeSignalMapper, SIGNAL(mapped(int)), this, SLOT(getShapeIndex(int)));
}
/**
 * @brief setMaterialSens::updateShapeList
 * 更新形状算法列表
 */
void setMaterialSens::updateShapeList()
{
    QString strName;
    QString strShapeList;
    ...
    for (int i = 0; i < MAX_SHAPE; i++) {
        if (struCnfp.nArithmeticEnable[MAX_GENERAL+i]) {
            ...
            case 0:    //! 形状算法
                ...
                /* 隐藏后面按钮 */
//                shapeRowLabel[i]->setMaximumWidth(pageShape->width()/6);
//                shapeRowLbe[i]->setMaximumWidth(pageShape->width()/6);
                shapeRowLabel[i]->setEnabled(false);
                shapeRowLbe[i]->setEnabled(false);
                shapePercentLabel[i]->setEnabled(false);
                shapePercentLbe[i]->setEnabled(false);
                shapeRowLabel[i]->hide();
                shapeRowLbe[i]->hide();
...
//                shapeRowLbe[i]->setMaximumWidth(pageShape->width()/6);
            ...
            case 2:     //! 大小算法
                ...
                strShapeSens = QString("%1").arg(struCnfp.struGroupIdentify[struGsh.nLevel][currentChan].struScale[0].nValue);
                shapeSensLbe[i]->setText(strShapeSens);
...
//                shapeRowLbe[i]->setMaximumWidth(pageShape->width()/6);
    ...
    if (struCnfp.nArithmeticEnable[ARITH_SCALE_B]) {
        ...
        strShapeSens = QString("%1").arg(struCnfp.struGroupIdentify[struGsh.nLevel][currentChan].struScale[1].nValue);
        shapeSensLbe[5]->setText(strShapeSens);

        /* 隐藏后面按钮 */
//        shapeRowLabel[5]->setMaximumWidth(pageShape->width()/6);
//        shapeRowLbe[5]->setMaximumWidth(pageShape->width()/6);
        shapeRowLabel[5]->setEnabled(false);
        shapeRowLbe[5]->setEnabled(false);
        shapePercentLabel[5]->setEnabled(false);
        shapePercentLbe[5]->setEnabled(false);
        shapeRowLabel[5]->hide();
        shapeRowLbe[5]->hide();
        shapePercentLabel[5]->hide();
        shapePercentLbe[5]->hide();
    ...
    if(struCnfp.nArithmeticEnable[ARITH_CANDY])
    {
        ...
        shapePercentLabel[6]->setText(myLan.scale);
...
/**
 * @brief setMaterialSens::getShapeIndex
 * @param index
 * 修改形状算法参数
 */
...
/**
 * @brief setMaterialSens::createPeanutPage
 * 创建花生定制算法页面
 */
void setMaterialSens::createPeanutPage()
{
    ...
    /*槽函数*/
    connect(peanutSignalMapper, SIGNAL(mapped(int)), this, SLOT(getPeanutIndex(int)));
}

/**
 * @brief setMaterialSens::updatePeanutList
 * 更新花生定制算法列表
 */
...
/**
 * @brief setMaterialSens::getPeanutIndex
 * @param index
 * 修改花生定制算法各参数值
 */
void setMaterialSens::getPeanutIndex(int index)
{
    int ret;
    inputType type;
    int nMin,nMax;
...
/**
 * @brief setMaterialSens::createAIPage
 * 创建智能页面
 */
void setMaterialSens::createAIPage()
{
    AISignalMapper = new QSignalMapper(pageAI);
    m_AIRsvSigMap = new QSignalMapper(pageAI);
    ...
    for (int i = 0; i < MAX_AI;i++) {
        ...
        AIHBoxLayout[i]->addWidget(AIRowLabel[i]);
        AIHBoxLayout[i]->addWidget(AIRowLbe[i]);
        AIHBoxLayout[i]->addWidget(AIPercentLabel[i]);
        AIHBoxLayout[i]->addWidget(AIPercentLbe[i]);
        ...
        AIRatioHBoxLayout[i]->addStretch();
        ...
        if (myFlow.getProductLineNo() != 0) {   //非杂粮机型不显示
            ...
            AIModeGeneralTypeBtnLabel[i]->hide();
            AISensRatioLbe[i]->hide();
        }
    ...
    /* 槽函数 */
    connect(AISignalMapper, SIGNAL(mapped(int)), this, SLOT(getAIIndex(int)));
    connect(m_AIRsvSigMap, SIGNAL(mapped(int)), this, SLOT(onAIRsvBtnsPressed(int)));
}
/**
 * @brief setMaterialSens::updateAIList
 * 更新智能参数列表
 */
void setMaterialSens::updateAIList()
{
    QString strAIRow;
    QString strAIPercent;
    QString strAIBalance;
    ...
    for (int i = 0; i < MAX_AI; i++) {
        if (struCnfp.nArithmeticEnable[ARITH_INTEL_A+i]) {
            ...
            strAIPercent = QString("%1").arg(struCnfp.struGroupIdentify[struGsh.nLevel][currentChan].struIntel[i].nPercent);
            AIPercentLbe[i]->setText(strAIPercent);

            strAIBalance = QString("%1").arg(struCnfp.struGroupIdentify[struGsh.nLevel][currentChan].struIntel[i].nEnable);
            AIBalanceLbe[i]->setText(strAIBalance);
            ...
            case 0:
                ...
                break;
            case 1:
                AIModeBtn[i]->setText(myLan.reverse);
                break;
            case -1:
                AIModeBtn[i]->setText(myLan.unused);
                break;
            default:
                AIModeBtn[i]->setText(myLan.sort);
                break;
            ...
            if (myFlow.getProductLineNo() == 0) {
                ...
                AIModeGeneralTypeBtnLabel[i]->show();
                AISensRatioLbe[i]->show();
            }
...
/**
 * @brief setMaterialSens::getAIIndex
 * @param index
 * 修改智能参数
 */
void setMaterialSens::getAIIndex(int index)
{
...
/**
 * @brief setMaterialSens::createWatermelonPage
 * 创建西瓜定制算法页面
 */
void setMaterialSens::createWatermelonPage()
{
    ...
    for (int i = 0; i < MAX_WATERMELON; i++) {
        ...
        watermelonSignalMapper->setMapping(watermelonLineEdit[i], i);
        connect(watermelonLineEdit[i], SIGNAL(pressed()), watermelonSignalMapper, SLOT(map()));
    }

    watermelonLabel[0]->setText(myLan.material_watermelon_red_ratio);
    watermelonLabel[1]->setText(myLan.material_watermelon_red_sens);
    watermelonLabel[2]->setText(myLan.material_watermelon_damaged_ratio);
    watermelonLabel[3]->setText(myLan.material_watermelon_damaged_sens);
    watermelonLabel[4]->setText(myLan.material_watermelon_rugged1);
    watermelonLabel[5]->setText(myLan.material_watermelon_rugged_save_white);
    watermelonLabel[6]->setText(myLan.material_watermelon_white_sens);
    watermelonLabel[7]->setText(myLan.material_watermelon_rugged2);
    watermelonLabel[8]->setText(myLan.material_watermelon_rugged_save_black);

    watermelonLineEdit[0]->setText(QString("%1").arg(struCnfp.struGroupIdentify[0][0].struMatMelon.nEdgeRedRatio));
    ...
    watermelonLineEdit[8]->setText(QString("%1").arg(struCnfp.struGroupIdentify[0][0].struMatMelon.nRugged2Save));


    watermelonLayout->addWidget(watermelonLabel[0], 0, 0, 1, 1, Qt::AlignRight);
    watermelonLayout->addWidget(watermelonLineEdit[0], 0, 1, 1, 1, Qt::AlignLeft);
    watermelonLayout->addWidget(watermelonLabel[1], 0, 2, 1, 1, Qt::AlignRight);
    watermelonLayout->addWidget(watermelonLineEdit[1], 0, 3, 1, 1, Qt::AlignLeft);

    watermelonLayout->addWidget(watermelonLabel[2], 1, 0, 1, 1, Qt::AlignRight);
    watermelonLayout->addWidget(watermelonLineEdit[2], 1, 1, 1, 1, Qt::AlignLeft);
    watermelonLayout->addWidget(watermelonLabel[3], 1, 2, 1, 1, Qt::AlignRight);
    watermelonLayout->addWidget(watermelonLineEdit[3], 1, 3, 1, 1, Qt::AlignLeft);

    watermelonLayout->addWidget(watermelonLabel[4], 2, 0, 1, 1, Qt::AlignRight);
...
//    watermelonLayout->addWidget(watermelonLineEdit[6], 2, 5, 1, 1, Qt::AlignLeft);
    ...
    watermelonLayout->addWidget(watermelonLabel[8], 3, 2, 1, 1, Qt::AlignRight);
    watermelonLayout->addWidget(watermelonLineEdit[8], 3, 3, 1, 1, Qt::AlignLeft);


    updateWatermelonList();

    /*槽函数*/
    connect(watermelonSignalMapper, SIGNAL(mapped(int)), this, SLOT(changeWatermelonValue(int)));
}

/**
 * @brief setMaterialSens::updateWatermelonList
 * 更新西瓜籽定制算法列表
 */
void setMaterialSens::updateWatermelonList()
{
    QString str;
...
/**
 * @brief setMaterialSens::changeWatermelonValue
 * @param index
 * 修改西瓜子定制算法各参数值
 */
void setMaterialSens::changeWatermelonValue(int index)
{
    double nValue;
    int ret = 0;
    QString str;
...
/**
 * @brief setMaterialSens::createTeaPage
 * 创建茶叶机型定制灵敏度页面
 */
void setMaterialSens::createTeaPage()
{
    ...
    for (int i = 0; i < ARITHMETIC_TOTAL; i++) {
        ...
        TeaBoxLayout[i]->addWidget(TeaPercentLabel[i]);
        TeaBoxLayout[i]->addWidget(TeaPercentLbe[i]);
        TeaBoxLayout[i]->addStretch();
        TeaBoxLayout[i]->addWidget(TeaBalanceLabel[i]);
        TeaBoxLayout[i]->addWidget(TeaBalanceLbe[i]);
        TeaBoxLayout[i]->addStretch();
        TeaBoxLayout[i]->addWidget(TeaWidthLabel[i]);
        TeaBoxLayout[i]->addWidget(TeaWidthLbe[i]);
        TeaBoxLayout[i]->addStretch();
        TeaBoxLayout[i]->addWidget(TeaAreaLabel[i]);
    ...
    /* 槽函数 */
    connect(TeaSignalMapper, SIGNAL(mapped(int)), this, SLOT(getTeaIndex(int)));
}
 ...
 * 更新茶叶机型定制灵敏度页面
 */
void setMaterialSens::updateTeaList()
{
    QString name;
    QString title;
    QString text;
    ...
    for (int i = 0; i < ARITHMETIC_TOTAL; i++) {
        ...
        TeaBalanceLbe[i]->hide();
        TeaWidthLabel[i]->hide();
        TeaWidthLbe[i]->hide();
        TeaAreaLabel[i]->hide();
        TeaAreaLbe[i]->hide();
        TeaModeBtn[i]->hide();
...
/**
 * @brief setMaterialSens::getTeaIndex
 * @param index
 * 修改茶叶机型定制页面算法各参数值
 */
void setMaterialSens::getTeaIndex(int index)
{
    inputType nType;
    double nMin, nMax, nValue;
    QString text;
    ...
    if (index%MAX_TEA == 0) {
        ...
        case ARITH_SHAPE: // 形状-选圆长
            ...
            break;
        case ARITH_SHAPE_LENGTH: // 形状-选长短
            nMin = 0;
            nMax = 100;
            nType = floatType;
            ...
            break;
        case ARITH_SHAPE_POLE_A: // 形状-选粗细
        case ARITH_SHAPE_POLE_B:
            nMin = 0;
            nMax = 255;
            nType = intType;
        ...
        if (inputDlg1.exec() == QDialog::Accepted) {
            ...
            case ARITH_INTEL_A: // 智能
                ...
                break;
            case ARITH_SCALE: // 大小
                struCnfp.struGroupIdentify[struGsh.nLevel][currentChan].struScale[0].nValue = nValue;
                text = QString("%1").arg(nValue);
                break;
            case ARITH_SCALE_B: // 大小
                struCnfp.struGroupIdentify[struGsh.nLevel][currentChan].struScale[1].nValue = nValue;
                text = QString("%1").arg(nValue);
                break;
            case ARITH_SHAPE: // 形状
                if (struCnfp.struGroupIdentify[struGsh.nLevel][currentChan].struShape.nMode == 0) {
            ...
            case ARITH_SHAPE_POLE_A: // 选杆模式 - 细杆
                struCnfp.struGroupIdentify[struGsh.nLevel][currentChan].struPole.nArea = nValue;
                text = QString("%1").arg(nValue);
                break;
            case ARITH_SHAPE_POLE_B: // 选杆模式 - 粗杆
                struCnfp.struGroupIdentify[struGsh.nLevel][currentChan].struPole.nPolePercent = nValue;
                text = QString("%1").arg(nValue);
                break;
            case ARITH_SHAPE_SLICE: // 选片
                struCnfp.struGroupIdentify[struGsh.nLevel][currentChan].struSlice.nArea = nValue;
                text = QString("%1").arg(nValue);
                break;
            case ARITH_SHAPE_LEAF: // 反选选芽
                struCnfp.struGroupIdentify[struGsh.nLevel][currentChan].struLeaf.nThreshold = nValue;
                text = QString("%1").arg(nValue);
                break;
            default:
                break;
 ...
 * \brief 创建保留算法页面
 */
void setMaterialSens::createRsvPage()
{
    ...
    /*更新形状算法列表*/
    updateRsvList();
    connect(rsvSignalMapper, SIGNAL(mapped(int)), this, SLOT(getRsvIndex(int)));
}

/**
 * @brief setMaterialSens::updateRsvList
 * 更新保留算法页面
 */
void setMaterialSens::updateRsvList()
{
    QString strRsvList;
    QString strThresholdLabel_s;
...
/**
 * @brief setMaterialSens::getRsvList
 * 获取保留算法参数页面
 */
 ...
 * \brief 创建RSC机型的灵敏度页面
 */
void setMaterialSens::createPageRSC()
{
    m_scrollArea = new QScrollArea;
    m_scrollArea->setStyleSheet("width: 30");
