...
QDataBase::QDataBase()
{
    ...
    LOG_INFO_STM("create db:" << dir.absolutePath().toStdString() << ", db info:" << SYS_DB_INFO);
    ...
    if (!database.open())
    {
        LOG_ERROR_STM("Failed to connect database, err:" << database.lastError().text().toStdString());
