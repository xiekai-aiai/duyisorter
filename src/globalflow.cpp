...
void GlobalFlow::getGroupIdentify()
{
    ...
    for(i = 0; i < MAX_LEVEL; i++) {
        for (j = 0; j < MAX_GROUP_IDTNTIFY; j++) {
            for (m = 0; m < 4; m++) {	// 0: -grey A, 1: -grey B, 2: -discolor A, 3: -discolor B
...
void GlobalFlow::getDelayMaterialSize()
{
    ...
    for (i = 0; i < struCnfg.nLevelTotal; i++) {
        for (j = 0; j < struCnfg.struLevelInfo[i].nIdentifyGroupTotal; j++) {
            ...
            if (nArithmeticEnable[i][ARITH_CROSS] == 1) {   // 差分
            ...
            if (nArithmeticEnable[i][ARITH_CANDY]) {        // 糖果
            ...
            for (m = 0; m < 2; m++) {
                if (nArithmeticEnable[i][m+ARITH_BUD_1] == 1) {   // 选芽1、选芽2
            ...
            for (m = 0; m < 3; m++) {
                if (nArithmeticEnable[i][m+ARITH_PEANUT_A] == 1) {    // A、B、C类
                    nMaxValue = 1001;
            ...
            if (struCnfp.nIntelMode == 0) {  // 普通自学习
                for (m = 0; m < 4; m++) {
                    if (nArithmeticEnable[i][m+ARITH_INTEL_A] == 1) {     // 智能模式A、B、C、D
                ...
                if (myFlow.getProductLineNo() == 0) {
					for (m = 0; m < 2; m++) {
                        if (nArithmeticEnable[i][ARITH_INTEL_A+m] == 1) {     // 智能模式
                            if (struCnfp.struGroupIdentify[i][j].struIntel[m].nIntelType == 0) {// 普通智能模式
            ...
            if (((nArithmeticEnable[i][ARITH_SHAPE] == 1)
                    ...
                    || (nArithmeticEnable[i][ARITH_SCALE_B] == 1)) { // 形状算法+大小算法
                nMaxSize[i][j] = (nMaxSize[i][j] < 3) ? 3 : nMaxSize[i][j];
            }

            if ((nArithmeticEnable[i][ARITH_RESERVED] == 1) || nArithmeticEnable[i][ARITH_MAIZE] == 1) { // 玉米算法或者保留算法
                nMaxSize[i][j] = (nMaxSize[i][j] < 10) ? 10 : nMaxSize[i][j];
            }
            if (nArithmeticEnable[i][ARITH_SHAPE_POLE_A] == 1 || nArithmeticEnable[i][ARITH_SHAPE_POLE_B] == 1) { // 选杆算法
                nMaxSize[i][j] = (nMaxSize[i][j] < 25) ? 25 : nMaxSize[i][j];
            }
            if (((nArithmeticEnable[i][ARITH_BUD_1] == 1 || nArithmeticEnable[i][ARITH_BUD_2]) == 1) && (struCnfc.nSensorType == SENSOR_T_2564)) { // 选芽算法
    ...
    for (i = 0; i < struCnfg.nLevelTotal; i++) {
        ...
        for (j = 0; j < struCnfg.struLevelInfo[i].nUnitLevelTotal; j++) {
            ...
            if (nGroupID > -1 && nGroupID < MAX_GROUP_IDTNTIFY) {
                if (struCnfp.struGroupIdentify[i][nGroupID].struIntel[0].nModeDfl == -1) {  // 智能A禁用
                    sParam2[i] &= 0xef;
                }

                if (struCnfp.struGroupIdentify[i][nGroupID].struIntel[1].nModeDfl == -1) {  // 智能B禁用
                    sParam2[i] &= 0xdf;
                }

                if (struCnfp.struGroupIdentify[i][nGroupID].struIntel[2].nModeDfl == -1) {  // 智能C禁用
                    sParam2[i] &= 0xbf;
                }

                if (struCnfp.struGroupIdentify[i][nGroupID].struIntel[3].nModeDfl == -1) {  // 智能D禁用
                    sParam2[i] &= 0x7f;
                }
            }
...
int GlobalFlow::getIdentifyGroupAddr(int nLevelId, int nGroupId, int nUnitIndex)
{
    ...
    if (struCnfg.struLevelInfo[nLevelId].struIdentifyGroupInfo[nGroupId].nUnitSplice[nUnitIndex] == 0) { //通道不按0.5：0.5分隔
        nUnitAddr = struCnfg.struLevelInfo[nLevelId].struIdentifyGroupInfo[nGroupId].nUnitId[nUnitIndex];
    } else if (struCnfg.struLevelInfo[nLevelId].struIdentifyGroupInfo[nGroupId].nUnitSplice[nUnitIndex] == 1) { //分割后左半部分
        nUnitAddr = 1 << 6 | struCnfg.struLevelInfo[nLevelId].struIdentifyGroupInfo[nGroupId].nUnitId[nUnitIndex];
    } else if (struCnfg.struLevelInfo[nLevelId].struIdentifyGroupInfo[nGroupId].nUnitSplice[nUnitIndex] == 2) { //分割后右半部分
...
void GlobalFlow::resetFeeder(int flag)
{
    ...
    //! CG 机型供料控制（带使能及供料偏置）
    {
        ...
        for (i = 1; i < nFeederTotal; i++) {
            if(struCnfg.nParamSetAlone == 1) {      //! 独立设置
...
void GlobalFlow::setCom2Baud()
{
    for (int i = 0; i < struCnfg.nCtrlBoardTotal; i++) {
        ...
        if (!checkCtrlBoardCom(i)) {     // 以115200查询
            MySerial.setComBaud(2, MY_UARTBAUD_921600);
            if (!checkCtrlBoardCom(i)) { // 以921600查询
                MySerial.setComBaud(2, MY_UARTBAUD_115200);
            } else {
        ...
        if (struCnfg.nEnableLampAI == 1) {
            if (struCnfe.nEnableCGMachine) {
                ...
                if (nUnitLevelTotal >= 16) {
                    ...
                    if (struCnfe.nEnableHGQMachine == 1){//黑枸杞定制
...
{
    int i, j;
    if (struGsh.bStatFeed == 1) {//关料
        struGsh.bStatFeed = 0;

        /* 记录日志 */
        myLog->info(LOG_FEED_SWITCH,"Feed off");
...
//            sleep(3);
        ...
        if (struGsh.bFlagMachinetest == 2) {    // 整机老化自检状态
            /* 记录日志 */
            myLog->info(LOG_FEED_SWITCH,"SelfExamine,Fre:%d",struGsh.nSelfExamineFre);
            ...
            /* 记录日志 */
            myLog->info(LOG_FEED_SWITCH,"Feed on");
            resetSortParams();

            for (i = 0; i < struCnfg.nInterfaceBoardTotal; i++) {    // 正常色选工作状态
...
void GlobalFlow::onOffMono()
{
    ...
    if (struGsh.bStatFeed == 1) {
        ...
        /* 记录日志 */
        myLog->info(LOG_FEED_SWITCH, "Feed on");
        myLog->info(LOG_PROFILE, "Scheme name: %s, file: %s",
